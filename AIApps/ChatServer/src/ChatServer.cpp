...
#include "../include/handlers/ChatNewSessionHandler.h"
...
void ChatServer::initialize() {
    ...
    const std::string mysqlHost = envOr("MYSQL_HOST", "127.0.0.1");
    const std::string mysqlPort = envOr("MYSQL_PORT", "3306");
    const std::string mysqlUser = envOr("MYSQL_USER", "root");
    const std::string mysqlPassword = envOr("MYSQL_PASSWORD", "123456");
    const std::string mysqlDatabase = envOr("MYSQL_DATABASE", "ChatHttpServer");
    http::MysqlUtil::init(
    ...
    FileUtil::setDefaultFile(chatResourceFile("NotFound.html"));
    rag::LocalKnowledgeBase::instance().loadFromConfig(chatResourceFile("config.json"));

    try {
        const char* baiduId = std::getenv("BAIDU_CLIENT_ID");
        const char* baiduSecret = std::getenv("BAIDU_CLIENT_SECRET");
        if (baiduId && baiduSecret && *baiduId && *baiduSecret) {
            AISpeechProcessor warmer(baiduId, baiduSecret);
            warmer.warmup();
            std::cout << "Baidu TTS token warmed up" << std::endl;
        }
    } catch (const std::exception& e) {
        std::cerr << "Baidu TTS warmup skipped: " << e.what() << std::endl;
    }
}
...
std::pair<std::string, std::shared_ptr<AIHelper>> ChatServer::createEmptySession(int userId) {
    ...
    chatInformation[userId].emplace(sessionId, helper);
    std::lock_guard<std::mutex> lockIds(mutexForSessionsId);
    sessionsIdsMap[userId].push_back(sessionId);
    return { sessionId, helper };
}
...
void ChatServer::initializeRouter() {
    ...
    httpServer_.Get("/kb", std::make_shared<KbPageHandler>(this));
    httpServer_.Get("/kb/list", std::make_shared<KbListHandler>(this));
    httpServer_.Post("/kb/upload", std::make_shared<KbUploadHandler>(this));
    httpServer_.Post("/kb/search", std::make_shared<KbSearchHandler>(this));
    ...
    httpServer_.Post("/chat/new-session", std::make_shared<ChatNewSessionHandler>(this));
