...
void ChatHistoryHandler::handle(const http::HttpRequest& req, http::HttpResponse* resp)
{
    try
    {
...

        {
            ...
            pendingQuestion = AIHelperPtr->pendingQuestion();
        }
        ...
        // 模型还在回答时告诉前端，离开页面再回来才能接着显示“正在思考”
        successResp["pending"] = !pendingQuestion.empty();
        successResp["pendingQuestion"] = pendingQuestion;
